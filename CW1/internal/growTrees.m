...
plotbagging = 0;
ploteg = 0;
igs = zeros(1,2^(param.depth-1)-1);
time1 = tic;
for T = 1:param.num
    
    % Bootstraping aggregating
    % A new training set for each tree is generated by random sampling from dataset WITH replacement.
    idx = randsample(N,ceil(N*frac),1); % frac = 1 for ~63% unique data
    if ploteg
        figure('rend','painters','pos',[100 100 450 300]);
        plot_toydata(data(idx, :),'train')
        nunique = 100*length(unique(idx))/length(idx);
        title(sprintf('Set %i: %.1f%% Unique',T, nunique));
    end
    % Evaluate uniqueness for various frac
    if plotbagging
        graphdata = [];
        for i = 1:N
            sz = 100;
            tmp = zeros(1,sz);
            for j = 1:sz
                idx = randsample(N,ceil(N*i/N),1);
                tmp(j) = 100*length(unique(idx))/length(idx);
            end
            nUnique = mean(tmp);
            graphdata = [graphdata; [i, nUnique]];
        end
        figure;
        plot(graphdata(:,1), graphdata(:,2));
        title('Average Uniqueness Over 100 Realisations');
        xlabel('N_T');
        ylabel('%% Unique data');
    end
    ...
    for n = 1:2^(param.depth-1)-1
        ...
        [tree(T).node(n),tree(T).node(n*2),tree(T).node(n*2+1), IG] = splitNode(data,tree(T).node(n),param);
        if ~isinf(IG)
            igs(n) = igs(n) + IG;
        end
    end    
...
time = toc(time1);
avgTime = time/param.num;
avgIG = sum(igs)/param.num;
end